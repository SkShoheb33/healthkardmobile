...
function HospitalCard({ hospital, address = false, horizontal = false }) {
    ...
    return (
        <Pressable
            ...
            className={`border-1 ${horizontal ? 'w-[210]' : 'w-1/2'} items-start border border-gray-300 p-1 rounded-md shadow-xl bg-white`}
        >
            <Image
                ...
                className='w-full'
            />
            <Text className='font-semibold text-lg my-1 text-black text-ellipsis overflow-hidden'>
            ...
            <View className='flex-row flex-wrap items-center'>
                {hospital.hospitalDetails.servicesOffered.length > 2 ? (
                    <View>
                        {hospital.hospitalDetails.servicesOffered.slice(0, 2).map((service, index) => (
                            <Service key={index} service={service} />
                        ))}
                        <Service service={`${hospital.hospitalDetails.servicesOffered.length - 2}+ more`} />
                    </View>
                ) : (
