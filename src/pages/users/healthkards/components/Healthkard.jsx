        ...
        <View style={ { flex: 1 } }>
            ...
            <View style={ { flex: 1 } } className='relative flex h-full justify-center items-center bg-white'>
                { loading ? <View style={ styles.lightGreen } className='w-10/12 flex flex-col justify-center items-center p-4 rounded-md shadow-xl'>
                    ...
                    <Text className='text-2xl font-bold text-black'>{ kard.healthId }</Text>
                    <View className='my-6'>
                        <Text className='my-1 text-black'>{ kard.name }</Text>
                        <Text className='my-1 text-black'>{ kard.gender }, { kard.age }</Text>
                        { kard.email && <Text className='my-1 text-black'>{ kard.email }</Text> }
                        { kard.number && <Text className='my-1 text-black'>{ `+${kard.number.slice(0, 2)} ${kard.number.slice(2)}` }</Text> }
                        <Text className='my-1 text-black'>{ kard.address }, { kard.city }, { kard.pincode }</Text>
                        <Text className='my-1 text-black'><Text style={ styles.blueText }>Last Plan : </Text>{ kard.lastPlan }</Text>
                        <View className='flex flex-row items-center justify-between'>
                            <Text className='my-1 text-black'><Text style={ styles.blueText }>Validity Till :</Text> { kard.paymentStatus ? <Text>{ (new Date(kard.expireDate)).toLocaleDateString() }</Text> : <Text className='text-red-500'>Invalid</Text> }</Text>
