...
function Button({ color, label, onPress, style = 'w-full p-4', transparent = false, icon, iconColor = '#000', disabled = false, iconSize = 12, iconPosition = 'left' }) {
  return (
    <TouchableOpacity onPress={ !disabled ? onPress : null } style={ !transparent && (color === 'blue' ? styles.blue : styles.green) } className={ `font-semibold ${style}  mx-auto rounded items-center my-2 ${iconPosition === 'right' ? 'flex-row-reverse' : 'flex-row'} justify-center ${disabled && 'opacity-50'}` }>
      { icon &&
